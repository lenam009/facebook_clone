...
class UserController {
    ...
    async follow(req, res, next) {
        if (req.body._id !== req.params._id || req.body.isAdmin) {
            ...
            const users = await Promise.all([User.findById(req.params._id), User.findById(req.body._id)])
            ...
            if (!userCurrent.followings.includes(user._id)) {
                const userUpdate = user.updateOne({ $push: { followers: userCurrent._id } });
                const userCurrentUpdate = userCurrent.updateOne({ $push: { followings: user._id } });
    ...
    async unfollow(req, res, next) {
        if (req.body._id !== req.params._id || req.body.isAdmin) {
            ...
            //Xét 2 trường hợp đã follow hoặc chưa follow
            if (userCurrent.followings.includes(user._id)) {
                const userUpdate = user.updateOne({ $pull: { followers: userCurrent._id } });
                const userCurrentUpdate = userCurrent.updateOne({ $pull: { followings: user._id } });
                Promise.all([userUpdate, userCurrentUpdate])
